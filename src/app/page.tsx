...
export default async function Home() {
  ...
  // const highlightedArticles = await Article.get({limit: 4});
  // const articles = await Article.get({limit: 10, offset: 4});
              ...
              return (
                <div key={article.title} className="flex bg-slate-800 rounded-md py-4">
                  <div className='flex items-center'>
                    <div className='h40 rounded-r-lg overflow-hidden'>
                      <Image 
                        className='h-full w-full object-cover transition duration-500 hover:scale-105 rounded-r-lg'
                        src={`/assets/images/articles/${article.image}`} 
                        alt={article.title} 
                  ...
                  <div className="w-full flex flex-col gap-2 pl-4">
                    {/* <h2 className="text-3xl mb-4 text-indigo-400/90">{article.id} - {article.title}</h2> */}
                    <h2 className="text-3xl mb-4 text-indigo-400/90">{article.title}</h2>
